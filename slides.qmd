...
\
Packages like jQuery and D3 provide convenient wrappers around built-in features such as `document.querySelectorAll()`. They may save some typing, but you should also learn how to DOM manipulation yourself.
...
My example data came on a convenient scale. Usually you will need a scale mapping from data to position or color.
...
* D3 has a helper to draw a scale axis and ticks.
...
\
In both cases, I specify to listen to the address `127.0.0.1` which is accessable from your own computer but not the local network.
