...
Create an HTML page with these tags:
...
Now we add some Javascript. We'll be using the D3 package. We'll load it over the web for now. Later we'll load it from a local file.

D3 will give us an way to alter our document. (It's also not hard to do this without D3.)
...
* `e => { ... }` is a concise way to create a function. This function takes one argument, `event`.
...
First let's load the data. We will also load a local copy of D3.

Your HTML will have to be in the same directory as the files `spots.js` and `d3.js`.
...
Using a local copy of D3 means we are not dependent on an external website. This may help the longevity of our page.
